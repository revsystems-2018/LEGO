...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DWCare4You ETL', 
		@enabled=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Pull Care4You Data', 
		@step_id=1, 
		...
		@command=N'EXECUTE [ETL].[PullDataToDatawarehouse] @table = ''Employee'';
...
EXECUTE [ETL].[PullDataToDatawarehouse] @table = ''Eventuality'';', 
		@database_name=N'Care4You', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback

EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Merge Care4You and DWCare4You Data', 
		@step_id=2, 
		@cmdexec_success_code=0, 
		@on_success_action=3, 
		@on_success_step_id=0, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Clean Staging Area', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'TRUNCATE TABLE [ETL].[Employee];
